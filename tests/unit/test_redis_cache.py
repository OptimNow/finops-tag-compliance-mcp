...
class TestRedisCacheInitialization:
    ...
    def test_init_with_valid_url(self, mock_redis):
    ...
    @patch('mcp_server.clients.cache.redis.from_url')
    def test_init_with_custom_ttl(self, mock_redis):
    ...
    @patch('mcp_server.clients.cache.redis.from_url')
    def test_init_connection_failure(self, mock_redis, mock_logger):
    ...
    @patch('mcp_server.clients.cache.redis.from_url')
    def test_is_connected_when_connected(self, mock_redis):
    ...
    @patch('mcp_server.clients.cache.logger')
    @patch('mcp_server.clients.cache.redis.from_url')
    def test_is_connected_when_disconnected(self, mock_redis, mock_logger):
...
class TestRedisCacheGet:
    ...
    async def test_get_existing_key(self, mock_redis):
        ...
        mock_client.get = MagicMock(return_value=json.dumps(test_data))
        mock_redis.return_value = mock_client
    ...
    async def test_get_connection_error(self, mock_redis, mock_logger):
        ...
        mock_client = MagicMock()
        mock_client.ping = MagicMock()
        mock_client.get = MagicMock(side_effect=RedisConnectionError("Connection lost"))
        mock_redis.return_value = mock_client
...
class TestRedisCacheSet:
    ...
    async def test_set_connection_error(self, mock_redis, mock_logger):
        ...
        mock_client = MagicMock()
        mock_client.ping = MagicMock()
        mock_client.setex = MagicMock(side_effect=RedisConnectionError("Connection lost"))
        mock_redis.return_value = mock_client
