...
async def test_get_ec2_instances_mixed_states():
    ...
    with mock_aws():
        ...
        stopped_instances = ec2.create_instances(
            ImageId="ami-12345678", MinCount=1, MaxCount=1, InstanceType="m5.large"
        )
...
async def test_get_opensearch_domains_describe_error():
    ...
    with patch.object(client, "_get_account_id", new_callable=AsyncMock) as mock_account:
        ...
        async def mock_call_side_effect(service_name, func, *args, **kwargs):
            ...
            if call_count[0] == 1:  # list_domain_names
                return {
                    "DomainNames": [{"DomainName": "good-domain"}, {"DomainName": "bad-domain"}]
                }
            ...
            "ResourceTagMappingList": [
                {
                    ...
                    "Tags": [{"Key": "Application", "Value": "backend-api"}],
                }
            ]
        }
            ...
            "ResourceTagMappingList": [
                {
                    ...
                    "Tags": [{"Key": "Team", "Value": "notifications"}],
                }
            ]
        }
            ...
            "ResourceTagMappingList": [
                {
                    ...
                    "Tags": [{"Key": "Purpose", "Value": "order-processing"}],
                }
            ]
        }
...
async def test_get_all_tagged_resources_with_resource_type_filter():
    ...
    with patch.object(client, "_call_with_backoff", new_callable=AsyncMock) as mock_call:
        ...
        resources = await client.get_all_tagged_resources(resource_type_filters=["ec2:instance"])
...
async def test_get_all_tagged_resources_without_tags():
    ...
    with patch.object(client, "_call_with_backoff", new_callable=AsyncMock) as mock_call:
        mock_call.return_value = {
            "ResourceTagMappingList": [
                {"ResourceARN": "arn:aws:ec2:us-east-1:123456789012:instance/i-12345", "Tags": []}
            ]
        }
