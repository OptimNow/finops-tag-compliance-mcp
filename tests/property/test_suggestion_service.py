...
ENVIRONMENT_KEYWORDS = ["prod", "prd", "production", "stag", "stg", "staging", "dev", "development", "test", "qa"]

# Cost center keywords
COST_CENTER_KEYWORDS = ["eng", "engineering", "mkt", "marketing", "sales", "ops", "operations", "fin", "finance"]
...
non_empty_string = st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('L', 'N'), whitelist_characters='-_'))
...
resource_name_with_env_strategy = st.builds(
    ...
    suffix=st.text(min_size=2, max_size=8, alphabet=st.characters(whitelist_categories=('Ll', 'Nd')))
)
...
similar_resource_strategy = st.fixed_dictionaries({
    "resource_id": st.text(min_size=5, max_size=20, alphabet=st.characters(whitelist_categories=('Ll', 'Nd'), whitelist_characters='-')),
    "resource_type": resource_type_strategy,
    "tags": st.dictionaries(
        keys=st.sampled_from(["Environment", "CostCenter", "Owner", "Application"]),
        values=st.text(min_size=1, max_size=30, alphabet=st.characters(whitelist_categories=('L', 'N'), whitelist_characters='-_@.')),
        min_size=1,
        max_size=4
    )
})
...
class TestSuggestionQuality:
    ...
    def test_environment_suggestions_have_valid_structure(
        ...
        if env_suggestions:
            ...
            assert any(word in suggestion.reasoning.lower() for word in ["pattern", "detected", "found", "extracted"])
    ...
    def test_iam_role_based_suggestions_include_reasoning(
        ...
        if cost_center_suggestions:
            ...
            assert len(suggestion.reasoning) > 0
            assert any(word in suggestion.reasoning.lower() for word in ["pattern", "detected", "iam", "role"])
    ...
    def test_no_suggestions_for_already_tagged_resources(
        ...
        for tag in required_tags:
            if tag.allowed_values:
                ...
                current_tags[tag.name] = "test-value@example.com" if tag.name == "Owner" else "test-app"
...
class TestSuggestionPatternMatching:
    ...
    def test_cost_center_pattern_detection(
        ...
        # Suggested value should be one of the allowed values
        assert suggestion.suggested_value in ["Engineering", "Marketing", "Sales", "Operations", "Finance"]
