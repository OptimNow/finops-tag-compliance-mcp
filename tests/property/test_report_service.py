...
def violation_strategy():
    ...
    return st.builds(
        ...
        allowed_values=st.one_of(
            st.none(), st.lists(st.text(min_size=1, max_size=20), min_size=1, max_size=5)
        ),
        cost_impact_monthly=st.floats(
...
class TestReportContentCompleteness:
    ...
    def test_report_includes_compliance_score(
        ...
        ), "Report missing overall_compliance_score"
        assert (
            report.overall_compliance_score == score
        ), f"Report score {report.overall_compliance_score} != expected {score}"
        assert (
            0.0 <= report.overall_compliance_score <= 1.0
    ...
    def test_report_includes_violations_ranked_by_count(
        ...
        for i in range(len(report.top_violations_by_count) - 1):
            assert (
                report.top_violations_by_count[i].violation_count
                >= report.top_violations_by_count[i + 1].violation_count
            ), (
    ...
    def test_report_includes_violations_ranked_by_cost(
        ...
        for i in range(len(report.top_violations_by_cost) - 1):
            assert (
                report.top_violations_by_cost[i].total_cost_impact
                >= report.top_violations_by_cost[i + 1].total_cost_impact
            ), (
    ...
    @given(
        ...
        cost_gap=st.floats(
            min_value=2000.0, max_value=10000.0, allow_nan=False, allow_infinity=False
        ),
    )
    @settings(max_examples=100)
    ...
    def test_report_total_violations_matches_input(
        ...
        assert (
            report.total_violations == num_violations
    ...
    def test_violation_rankings_contain_required_fields(
        ...
        for ranking in report.top_violations_by_count + report.top_violations_by_cost:
            ...
            assert (
                hasattr(ranking, "violation_count") and ranking.violation_count >= 0
            ), "Ranking missing violation_count"
            assert (
                hasattr(ranking, "total_cost_impact") and ranking.total_cost_impact >= 0
            ), "Ranking missing total_cost_impact"
            assert hasattr(
                ranking, "affected_resource_types"
            ), "Ranking missing affected_resource_types"
            assert isinstance(
                ranking.affected_resource_types, list
            ), "affected_resource_types must be a list"
