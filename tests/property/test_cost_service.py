...
def create_cost_service():
    ...
    return CostService(
        aws_client=create_mock_aws_client(), policy_service=create_mock_policy_service()
    )
...
account_strategy = st.sampled_from(["111111111111", "222222222222", "333333333333", "444444444444"])
...
class TestCostAttributionCalculation:
    ...
    def test_attribution_gap_equals_total_minus_attributable(
        ...
        result = CostAttributionResult(
            ...
            attribution_gap_percentage=(
                (expected_gap / total_spend * 100) if total_spend > 0 else 0.0
            ),
        )

        # Verify the gap calculation
        assert (
            abs(result.attribution_gap - (result.total_spend - result.attributable_spend)) < 1e-10
        ), (
    ...
    def test_gap_percentage_equals_gap_over_total_times_100(
        ...
        if result.total_spend > 0:
            ...
            assert abs(result.attribution_gap_percentage - expected) < 1e-10, (
                f"Gap percentage {result.attribution_gap_percentage} != " f"expected {expected}"
            )
    ...
    def test_grouped_gaps_sum_equals_total_gap(
        ...
        assert abs(sum_of_grouped_gaps - result.attribution_gap) < 1e-6, (
            f"Sum of grouped gaps {sum_of_grouped_gaps} != " f"total gap {result.attribution_gap}"
        )
    ...
    def test_region_grouping_gaps_sum_equals_total(
        ...
        assert abs(sum_of_grouped_gaps - result.attribution_gap) < 1e-6, (
            f"Sum of region gaps {sum_of_grouped_gaps} != " f"total gap {result.attribution_gap}"
        )
    ...
    def test_account_grouping_gaps_sum_equals_total(
        ...
        assert abs(sum_of_grouped_gaps - result.attribution_gap) < 1e-6, (
            f"Sum of account gaps {sum_of_grouped_gaps} != " f"total gap {result.attribution_gap}"
        )
    ...
    def test_all_attributable_means_zero_gap(
        ...
        assert result.attribution_gap == 0.0, (
            f"Expected 0.0 gap when all spend is attributable, got " f"{result.attribution_gap}"
        )
        assert result.attribution_gap_percentage == 0.0, (
    ...
    def test_grouped_totals_sum_equals_total_spend(
        ...
        assert abs(sum_of_grouped_totals - result.total_spend) < 1e-6, (
            f"Sum of grouped totals {sum_of_grouped_totals} != " f"total spend {result.total_spend}"
        )
