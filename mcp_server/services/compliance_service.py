...
class ComplianceService:
    ...
    async def _scan_and_validate(
        ...
        """
        logger.info(f"Scanning resources: {resource_types}, filters: {filters}, severity: {severity}")
        ...
        logger.info(f"Found {len(all_violations)} violations across {len(filtered_resources)} resources")
    ...
    def _apply_resource_filters(
        ...
        if "region" in filters and filters["region"]:
            ...
            filtered = [
                r for r in filtered
                if r.get("region") in region_filter
            ]
    ...
    async def invalidate_cache(
        ...
        if resource_types is None and filters is None:
            ...
            cache_key = self._generate_cache_key(
                resource_types or [],
                filters,
                "all"  # Default severity for invalidation
            )
