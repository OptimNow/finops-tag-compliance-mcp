...
class ReportService:
    ...
    def generate_report(
        ...
        """
        logger.info(f"Generating compliance report for {compliance_result.total_resources} resources")
    ...
    def _rank_violations_by_count(
        ...
        violation_groups = defaultdict(lambda: {
            "count": 0,
            "cost": 0.0,
            "resource_types": set()
        })
        ...
        for tag_name, data in violation_groups.items():
            rankings.append(ViolationRanking(
                tag_name=tag_name,
                violation_count=data["count"],
                total_cost_impact=data["cost"],
                affected_resource_types=sorted(list(data["resource_types"]))
            ))
    ...
    def _generate_recommendations(
        ...
        if top_by_count:
            ...
            if resource_type_violations:
                worst_resource_type = max(
                    resource_type_violations.items(),
                    key=lambda x: x[1]
                )
        ...
        if compliance_result.cost_attribution_gap > 5000:
            recommendations.append(ComplianceRecommendation(
                ...
                affected_resources=compliance_result.total_resources - compliance_result.compliant_resources
            ))
    ...
    def _format_as_csv(self, report: ComplianceReport) -> str:
        ...
        if report.recommendations:
            ...
            writer.writerow(["Priority", "Title", "Description", "Estimated Impact", "Affected Resources"])
