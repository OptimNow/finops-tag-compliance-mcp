...
class PolicyService:
    ...
    def __init__(self, policy_path: str | Path | None = None):
        ...
        self._policy_path = Path(policy_path) if policy_path else Path("policies/tagging_policy.json")
    ...
    def load_policy(self, policy_path: str | Path | None = None) -> TagPolicy:
        ...
        if not path.exists():
            raise PolicyNotFoundError(
                ...
                f"Please create a policy file at this location."
            )
        ...
        try:
            ...
            raise PolicyValidationError(
                f"Invalid JSON in policy file {path}: {e}"
            ) from e
        except Exception as e:
            raise PolicyValidationError(
                f"Error reading policy file {path}: {e}"
            ) from e
        ...
        try:
            ...
            raise PolicyValidationError(
                f"Invalid policy structure in {path}: {e}"
            ) from e
    ...
    def validate_tag_value(
        ...
        if isinstance(tag, RequiredTag) and tag.validation_regex is not None:
            try:
                ...
                if not pattern.match(tag_value):
                    return False, f"Value '{tag_value}' does not match required pattern: {tag.validation_regex}"
