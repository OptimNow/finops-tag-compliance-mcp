...
class ToolUsageStats(BaseModel):
    ...
    invocation_count: int = Field(
        default=0,
        ge=0,
        description="Total number of times this tool was invoked"
    )
    ...
    min_execution_time_ms: Optional[float] = Field(
        default=None,
        ge=0.0,
    ...
    last_error: Optional[str] = Field(
        default=None,
        description="Error message from the last failed invocation"
    )
...
class ErrorRateMetrics(BaseModel):
    ...
    overall_error_rate: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Overall error rate as a fraction (0.0 to 1.0)"
    )
    ...
    errors_by_tool: dict[str, int] = Field(
        default_factory=dict,
        description="Count of errors by tool name"
    )
    recent_errors: list[dict] = Field(
        default_factory=list,
        description="List of recent error events with timestamps and details"
    )
...
class BudgetUtilizationMetrics(BaseModel):
    ...
    session_id: Optional[str] = Field(
        default=None,
        description="Session identifier (None for global metrics)"
    )
    ...
    exhaustion_timestamp: Optional[datetime] = Field(
        default=None,
        description="Timestamp when budget was exhausted (if applicable)"
    )
    reset_timestamp: Optional[datetime] = Field(
        default=None,
        description="Timestamp when budget will reset (if applicable)"
    )
    active_sessions_count: int = Field(
        default=0,
        ge=0,
        description="Number of active sessions (for global metrics)"
    )
    sessions_exhausted_count: int = Field(
        default=0,
        ge=0,
        description="Number of sessions that have exhausted their budget"
    )
...
class LoopDetectionMetrics(BaseModel):
    ...
    active_sessions_with_loops: int = Field(
        default=0,
        ge=0,
        description="Number of active sessions currently tracking loops"
    )
    loops_by_tool: dict[str, int] = Field(
        default_factory=dict,
        description="Count of loops detected by tool name"
    )
    loops_by_session: dict[str, int] = Field(
        default_factory=dict,
        description="Count of loops detected by session ID"
    )
    last_loop_detected_at: Optional[datetime] = Field(
        default=None,
    ...
    last_loop_session_id: Optional[str] = Field(
        default=None,
        description="Session ID that triggered the last loop detection"
    )
    loop_prevention_enabled: bool = Field(
        default=True,
        description="Whether loop detection and prevention is enabled"
    )
    ...
    recent_loops: list[dict] = Field(
        default_factory=list,
        description="List of recent loop detection events with details"
    )
...
class SessionMetrics(BaseModel):
    ...
    tool_invocation_count: int = Field(
        default=0,
        ge=0,
        description="Total number of tool invocations in this session"
    )
    ...
    total_execution_time_ms: float = Field(
        default=0.0,
        ge=0.0,
        description="Total execution time for all tools in this session"
    )
    average_execution_time_ms: float = Field(
        default=0.0,
        ge=0.0,
        description="Average execution time per tool invocation"
    )
    budget_status: Optional[BudgetUtilizationMetrics] = Field(
        default=None,
        description="Current budget status for this session"
    )
    loop_detection_status: Optional[LoopDetectionMetrics] = Field(
        default=None,
        description="Loop detection status for this session"
    )
    tools_used: list[str] = Field(
        default_factory=list,
        description="List of unique tools invoked in this session"
    )
    error_count: int = Field(
        default=0,
        ge=0,
        description="Total number of errors in this session"
    )
...
class GlobalMetrics(BaseModel):
    ...
    server_start_time: datetime = Field(
        ...,
    ...
    total_tool_invocations: int = Field(
        default=0,
        ge=0,
        description="Total number of tool invocations across all sessions"
    )
    total_tool_successes: int = Field(
        default=0,
        ge=0,
        description="Total number of successful tool invocations"
    )
    total_tool_failures: int = Field(
        default=0,
        ge=0,
        description="Total number of failed tool invocations"
    )
    overall_error_rate: float = Field(
        default=0.0,
        ge=0.0,
    ...
    total_execution_time_ms: float = Field(
        default=0.0,
        ge=0.0,
        description="Total execution time across all tool invocations"
    )
    average_execution_time_ms: float = Field(
        default=0.0,
        ge=0.0,
    ...
    tool_stats: list[ToolUsageStats] = Field(
        default_factory=list,
        description="Usage statistics for each tool"
    )
    error_metrics: Optional[ErrorRateMetrics] = Field(
        default=None,
        description="Error rate metrics"
    )
    budget_metrics: Optional[BudgetUtilizationMetrics] = Field(
        default=None,
        description="Global budget utilization metrics"
    )
    loop_detection_metrics: Optional[LoopDetectionMetrics] = Field(
        default=None,
        description="Global loop detection metrics"
    )
    most_used_tool: Optional[str] = Field(
        default=None,
        description="Name of the most frequently used tool"
    )
    least_used_tool: Optional[str] = Field(
        default=None,
        description="Name of the least frequently used tool"
    )
