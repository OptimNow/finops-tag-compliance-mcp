...
class ViolationRanking(BaseModel):
    ...
    affected_resource_types: list[str] = Field(
        ...
        description="List of resource types affected by this violation"
    )
...
class ComplianceReport(BaseModel):
    ...
    cost_attribution_gap: float = Field(
        0.0,
        description="Total monthly cost that cannot be attributed",
        ge=0.0
    )
    ...
    top_violations_by_count: list[ViolationRanking] = Field(
        default_factory=list,
        description="Top violations ranked by occurrence count"
    )
    top_violations_by_cost: list[ViolationRanking] = Field(
        default_factory=list,
        description="Top violations ranked by cost impact"
    )
