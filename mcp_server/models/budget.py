...
class BudgetStatus(BaseModel):
    ...
    current_count: int = Field(
        ..., ge=0, description="Current number of tool calls made in this session"
    )
    max_calls: int = Field(..., ge=0, description="Maximum tool calls allowed per session")
    remaining: int = Field(..., ge=0, description="Remaining tool calls available")
    utilization_percent: float = Field(
        ..., ge=0.0, le=100.0, description="Percentage of budget used"
    )
    is_exhausted: bool = Field(..., description="Whether the budget is exhausted")
...
class BudgetExhaustedResponse(BaseModel):
    ...
    error_type: str = Field(
        default="budget_exhausted", description="Type of error for programmatic handling"
    )
    message: str = Field(..., description="Human-readable message explaining the budget exhaustion")
    session_id: str = Field(..., description="The session identifier that exhausted its budget")
...
class BudgetConfiguration(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Whether budget tracking is enabled")
    max_calls_per_session: int = Field(
        default=100, ge=1, description="Maximum tool calls allowed per session"
    )
    session_ttl_seconds: int = Field(
        default=3600, ge=60, description="TTL for session budget tracking in seconds"
    )
