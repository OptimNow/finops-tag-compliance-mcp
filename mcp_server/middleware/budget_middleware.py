...
class BudgetTracker:
    ...
    async def increment_count(self, session_id: str) -> int:
        ...
        if self._cache and self._cache._connected and self._cache._client:
            try:
                ...
                if new_count == 1:
                    await self._cache._client.expire(
                        key,
                        timedelta(seconds=self._session_ttl)
                    )
                return new_count
            except Exception as e:
    ...
    async def consume_budget(self, session_id: str) -> tuple[bool, int, int]:
        ...
        logger.debug(
            ...
            f"{new_count}/{max_calls} calls used"
        )
    ...
    async def get_active_session_count(self) -> int:
        ...
        if self._cache and self._cache._connected and self._cache._client:
            try:
                ...
                keys = []
                async for key in self._cache._client.scan_iter(
                    match=f"{self.BUDGET_KEY_PREFIX}*"
                ):
                    keys.append(key)
                return len(keys)
            except Exception as e:
