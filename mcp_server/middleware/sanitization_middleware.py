...
def sanitize_string(value: str, field_name: str = "input") -> str:
    ...
    for pattern in COMPILED_PATTERNS:
        if pattern.search(value):
            raise RequestSanitizationError(f"Suspicious pattern detected in {field_name}")
...
def validate_headers(headers: Headers) -> None:
    ...
    for name, value in headers.items():
        ...
        if name_lower in DANGEROUS_HEADERS:
            raise RequestSanitizationError(f"Dangerous header not allowed: {name}")

        # Check for CRLF injection in header values
        if "\r" in value or "\n" in value:
            raise RequestSanitizationError(f"Header injection detected in {name}")
...
def validate_url(url: str) -> None:
    ...
    if "../" in url or "..\\" in url:
        raise RequestSanitizationError("Path traversal attempt detected in URL")

    # Check for null bytes
    if "\x00" in url:
        raise RequestSanitizationError("Null byte detected in URL")
    ...
    if len(url) > MAX_PATH_LENGTH:
        raise RequestSanitizationError(f"URL path too long: {len(url)} (max: {MAX_PATH_LENGTH})")
        ...
        try:
            ...
            if content_length:
                try:
                    ...
                    raise RequestSanitizationError("Invalid Content-Length header")
...
def sanitize_json_value(value, field_path: str = "root") -> any:
    ...
    if isinstance(value, str):
        ...
        return {k: sanitize_json_value(v, f"{field_path}.{k}") for k, v in value.items()}
    elif isinstance(value, list):
        return [sanitize_json_value(item, f"{field_path}[{i}]") for i, item in enumerate(value)]
    else:
...
class RequestSizeLimits:
    ...
    def from_env(cls):
        ...
        cls.MAX_HEADER_SIZE_BYTES = int(os.getenv("MAX_HEADER_SIZE_BYTES", MAX_HEADER_SIZE_BYTES))
        cls.MAX_HEADER_COUNT = int(os.getenv("MAX_HEADER_COUNT", MAX_HEADER_COUNT))
        cls.MAX_QUERY_STRING_LENGTH = int(
            os.getenv("MAX_QUERY_STRING_LENGTH", MAX_QUERY_STRING_LENGTH)
        )
        cls.MAX_PATH_LENGTH = int(os.getenv("MAX_PATH_LENGTH", MAX_PATH_LENGTH))
